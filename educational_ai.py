...
def get_explanation(query):
 ...
 # print(potential_concepts)
  ...
  # If no perfect match, identify the concept with the most matching words (basic approach)
   ...
   summary = summarizer(explanation, num_sentences=2)  # Adjust num_sentences for desired summary length (default is 3)
   summarized_explanation = ' '.join(summary)  # Join summarized sentences

  # Build the response
  response = {
      "concept": best_match_concept,
      "explanation_summary": summarized_explanation,  # Use "explanation_summary" key for clarity
  }
...
"""
  ...
  explanation = knowledge_base[best_match_concept]["explanation"]

  # Apply summarization (adjust num_sentences as needed)
  summarizer = AutomaticSummarizer()
  summary = summarizer(explanation, num_sentences=2)  # Adjust num_sentences for desired summary length (default is 3)
  summarized_explanation = ' '.join(summary)  # Join summarized sentences
